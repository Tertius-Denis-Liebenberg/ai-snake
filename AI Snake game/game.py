...
# ----- Constants -----
BLOCK_SIZE = 20
SPEED_BASE = 40  # Slightly faster base speed for better feel
...
class SnakeGameAI():
    ...
    def play_step(self, action):
        ...
        if len(self.snake) >= self.max_capacity:
            if self.current_level < self.max_levels:
                self.current_level += 1
                reward += self.default_rewards['level_up']
    ...
    def get_fill_ratio(self):
        return float((len(self.snake) / self.max_capacity) * 100)
